...
function Guardian() {
  ...
  useEffect(() => {
  axios.get('http://localhost:8000/api/getGuardianApiSections', {
    headers: {
      ...
      .then(response => {
       ...
       setNewsSources(response.data.original);
      ...
      <form>
        <div className="row">
          <div className="col-md-4 mb-3 mb-md-0">
            ...
            <input id="search-input" className="form-control" type="text" value={searchKeyword} onChange={e => setSearchKeyword(e.target.value)} required />
          </div>
          <div className="col-md-4 mb-3 mb-md-0">
            <label htmlFor="from-date-input" className="form-label">From:</label>
            <input id="from-date-input" className="form-control" type="date" value={fromDate} onChange={e => setFromDate(e.target.value)}  required/>
          </div>
          <div className="col-md-4">
            <label htmlFor="to-date-input" className="form-label">To:</label>
            <input id="to-date-input" className="form-control" type="date" value={toDate} onChange={e => setToDate(e.target.value)}  required/>
          </div>
        </div>
      </form>
    </div>
  ...
  <button className="btn btn-primary mt-3" onClick={handleGetNews} disabled={loading}>{loading ? 'Loading...' : 'Get News'}</button>
      ...
      <div id="news">
        ...
        {news && news.map((selectedNews, index) => (
            <article key={index} >
              <img src={selectedNews?.fields.thumbnail} alt="" />
              <div className="text">
                <h1>{selectedNews?.webTitle}</h1>
                <p style={{ fontSize: '14px' }}>
                  {selectedNews?.description}
                  <a href={selectedNews?.webUrl} target="_blank">
                    <small>read more...</small>
                  </a>
                </p>
                <div style={{ paddingTop: '5px', fontSize: '12px' }}>Author: {selectedNews?.fields.byline ? selectedNews?.fields.byline : 'Unknown'}</div>
                {selectedNews?.webPublicationDate !== null ? (
                  <div style={{ paddingTop: '5px' }}>Date Published: {new Date(selectedNews?.webPublicationDate).toLocaleDateString()}</div>
                ) : (
